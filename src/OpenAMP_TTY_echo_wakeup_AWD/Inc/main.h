...
/**
  * @brief  Macro to calculate the ADC value (unit: digital value)
  *         corresponding to the provided voltage (unit: mVolt).
  * @note   The ADC value is on a 12 bit scale.
  * @note	The passed voltage needs to be in the interval (0, __VREFANALOG_VOLTAGE__).
  * @note   Analog reference voltage (Vref+) must be known from
  *         user board environment.
  * @param  __VREFANALOG_VOLTAGE__ Analog reference voltage (unit: mV)
  * @param  __VOLTAGE__ Voltage value (unit: mVolt).
  * @retval ADC value corresponding to the provided voltage (unit: mVolt).
  */

#define __ADC_CALC_VOLTAGE_DATA(__VREFANALOG_VOLTAGE__, __VOLTAGE__)		\
		((__VOLTAGE__) * DIGITAL_SCALE_12BITS / (__VREFANALOG_VOLTAGE__))
