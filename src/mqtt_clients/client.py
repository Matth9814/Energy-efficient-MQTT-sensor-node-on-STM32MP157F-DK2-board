    ...
    while not exitClient:
        ...
        if inp in ("y","yes"):
            ...
            try:
                ...
                print("\nStep 3. Set the wakeup wait time and the maximum update time")
                print(f">> Input format: <maxWakeupTime>{SEP}<maxUpdateTime>")
                print(">> 'maxWakeupTime' is the time waited for the boards to wakeup")
                print(">> 'maxUpdateTime' is the maximum time waited after sending the threshold to the boards")
                print(">> Insert an invalid input to exit")
                print("----> Wakeup/Update timeouts: ",end="")
                inp = input().strip().split(SEP)
                maxWuTime = int(inp[0])
                maxUpdTime = int(inp[1])
            except:
                print(">> Aborting threshold update...")
                boardsToUpdate = {UPDATE_KEYS[0]:{"boardsId":[],"isUpdated": []},UPDATE_KEYS[1]:""}
                continue
            # List of boards to wakeup/update
            boardsToWake = json.dumps({WAKEUP_KEYS[0]: boardsToUpdate[UPDATE_KEYS[0]]["boardsId"]})
            ...
            # Threshold update process start
            # Wake-up the boards (with magic-packets)
            # NOTE: Sending magic packets is not necessary when the board ETH wake-up mode is set to "u" (i.e. unicast)
            # This means that the code from this point, up to "thrUpdate = True", could be removed
            info = client.publish(PROXY_TOPIC_WAKEUP, payload=boardsToWake, qos=PROXY_QOS_WAKEUP)
            print("\n>> Board(s) wake-up: STARTED",end="")
            try:
                # TODO: check if waiting for the board wake-up before sending the threshold is necessary since the
                # topic has a QoS=2.
                # This couldn't be tested due to the ETH DMA reinitialization error after wake-up from suspension 

                # Wait for baords to wake-up
                # NOTE: Since it is not possible to accurately keep track of the boards status (i.e. awake or suspended),
                # the threshold is sent after 'maxWuTime' is waited. In case the message is not published the threshold update
                # process is suspended.
                startWuTime = time.time()
                info.wait_for_publish(maxWuTime) # Blocking call
            #    while time.time() - startWuTime > maxWuTime:
            #        pass
            except:
                print("\n>> Boards wake-up failed")
                boardsToUpdate = {UPDATE_KEYS[0]:{"boardsId":[],"isUpdated": []},UPDATE_KEYS[1]:""}
                continue
            print("\r>> Board(s) wake-up: FINISHED")
